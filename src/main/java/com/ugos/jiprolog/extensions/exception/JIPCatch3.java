...
public class JIPCatch3 extends JIPXCall
{
    ...
    public final synchronized boolean unify(final JIPCons params, Hashtable varsTbl)
    {
        ...
        {
            solution = m_recoverQuery.solveNext();
            if(solution != null)
            {
        ...
        try
        {
	        ...
	        if(m_jipQuery == null)
	        {
	            ...
	            m_jipQuery = m_engine.querySynch(goal);
	        }

            solution = m_jipQuery.solveNext();
            if(solution == null)
            {
                m_jipQuery.close();
                m_jipQuery = null;
        ...
        catch(JIPRuntimeException ex)
        {
            ...
            if(m_thrownTerm.unify(m_mask, varsTbl))
            {
                ...
                if (recoverGoal instanceof JIPVariable)
                {
                    ...
                    {
                        //extracts the term
                        recoverGoal = recoverGoal.getValue();
                    }
                }

                // Run recover goal
                m_recoverQuery = m_engine.querySynch(recoverGoal);
                solution = m_recoverQuery.solveNext();
