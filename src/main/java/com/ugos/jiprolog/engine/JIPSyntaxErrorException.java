...
//    }
    ...
    public JIPTerm getTerm()
    {
        if(m_engine != null && m_strFile != null)
        {
	        ...
	        try
	        {
	            return m_engine.parser().parseTerm("error(syntax_error(" + strTerm + "), file(" + strFile + ", " + m_nLine + "))");
	        }
	        catch (JIPSyntaxErrorException ex)
	        {
	             return m_engine.parser().parseTerm("error(syntax_error(undefined), file(undefined, undefined))");
	        }
        }
        else
        {
