...
//    public static final StringBuilderEx defaultStringBuilder = new StringBuilderEx();
    ...
    final boolean isMultifile(final String strPredName)
    {
        ...
        {
            return m_clauseTable.get(def).isMultifile();
        }
        else if(m_clauseTable.containsKey(strPredName))
        {
            return m_clauseTable.get(strPredName).isMultifile();
        }
...
//            throw JIPRuntimeException.create(46, Atom.createAtom(strPredName));
        ...
        JIPClausesDatabase db = m_clauseTable.get(sbUSER_MODULE.resetToInitialValue().append(strPredName).toString());
        if(db == null)
            db = m_clauseTable.get(sbSYSTEM_MODULE.resetToInitialValue().append(strPredName).toString());
    ...
    final boolean isModuleTransparent(final String strPredName)
    {
        ...
        if(m_clauseTable.containsKey(def))
        {
            return m_clauseTable.get(def).isModuleTransparent();
        }
    ...
    public final boolean isDynamic(final String strPredName)
    {
        ...
        if(m_clauseTable.containsKey(def))
        {
            return m_clauseTable.get(def).isDynamic();
        }
...
//        	System.out.println("not found " + strDef);
    ...
    public final boolean isExternal(final String strPredName)
    {
        ...
        if(m_clauseTable.containsKey(def))
        {
            return m_clauseTable.get(def).isExternal();
        }
...
//      System.out.println("more");
    ...
    private synchronized void addClause(final Clause clause, final boolean bFirst, final String strFile, boolean dynamic)
    {
        ...
        if(strFile != null)
        {
            ...
            {
                String strFileName = m_pred2FileMap.get(strFunctName);
...
//            System.out.println("database: " + strFunctName);
            ...
            // Estrae il database
            db = m_clauseTable.get(strFunctName);
...
//                    throw JIPRuntimeException.create(10, clause);
    ...
    final synchronized JIPClausesDatabase search(final Functor funct, final Stack<String> moduleStack)
    {
    	...
    	{
    		db = m_clauseTable.get(new StringBuilder(module).append(moduleFunName).toString());
    		if(db != null)
    			return db;
    	}

        db = m_clauseTable.get(sbUSER_MODULE.resetToInitialValue().append(funct.getName()).toString());//":" + funct.getName());
        if(db == null)
        {
        	db = m_clauseTable.get(sbSYSTEM_MODULE.resetToInitialValue().append(funct.getName()).toString());//":" + funct.getName());
        	if(db == null)
        	{
...
//        System.out.println("SearchFor: " + strModule + ":" + funct.getName());  // DBG
        ...
        JIPClausesDatabase db = m_clauseTable.get(new StringBuilder(strModule).append(':').append(funct.getName()).toString());
...
//            System.out.println("search in:" + USER_MODULE );  // DBG);
            ...
            db = m_clauseTable.get(sbUSER_MODULE.resetToInitialValue().append(funct.getName()).toString());
            if(db == null)
            {
    ...
    private static synchronized void loadKernel(GlobalDB gdb)
    {
    	try
        {
        	...
        	else
        	{
                 ...
                 if (ins == null)
                    throw new RuntimeException(KERNEL_RELEASE + " @" + gdb.getClass().getResource(KERNEL_RELEASE) + " missing");
